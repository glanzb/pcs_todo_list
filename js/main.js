...
function getFormValues(){
  ...
  var todo = form.elements.things.value;
  return {things: todo}; 
}
...
function addListItem( formValues){
  ...
  var node = document.createTextNode( formValues.things);
  ...
  deleteButtonImage.setAttribute("src", "http://www.iconsdb.com/icons/preview/black/minus-m.png");
  deleteButtonImage.setAttribute("alt", "[-]");
...
function makeStartingList(){
    for(var i = 0; i<1; i++){
    addListItem( {things: "These are all the things to do"});
    addListItem( {things: "And some more"});
    addListItem( {things: "You have to get them done"});
  }
}
